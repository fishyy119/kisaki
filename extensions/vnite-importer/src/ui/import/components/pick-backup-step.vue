...
const analysisStats = computed<readonly StatCard[]>(() => {
  ...
  return [
    ...
    { label: '诊断', value: analysis.diagnostics.errors + analysis.diagnostics.warnings, tone: 'warning' }
  ]
})
      ...
      >
        ...
        >
          <span class="inline-flex items-center justify-between gap-3">
            <span>备份分析发现 {{ props.analysis.diagnostics.errors }} 个错误，生成预览前建议先查看诊断。</span>
            <Button
              variant="text"
              size="xs"
              type="button"
        ...
        >
          <span class="inline-flex items-center justify-between gap-3">
            <span>备份分析发现 {{ props.analysis.diagnostics.warnings }} 个警告，导入时会尽量跳过受影响项目。</span>
            <Button
              variant="text"
              size="xs"
              type="button"
