...
function submit(): void {
  ...
  switch (current.step) {
    ...
    case 'config':
      ...
      return
    case 'preview':
      void runHostAction('start', () => host.startImport(snapshotOptions(), snapshotFieldSelection()))
      return
    case 'done':
...
<template>
  <div class="flex h-screen overflow-hidden bg-background text-foreground">
    ...
    <section class="flex min-w-0 flex-1 flex-col">
      <header class="flex min-h-14 items-center justify-between gap-3 border-b border-border px-4 py-2">
        <div class="min-w-0">
