...
export const zhHans: NeodbMessages = {
  errors: {
    ...
    instanceUnreachable: '无法连接 NeoDB 实例，请稍后重试。',
    instanceUnavailable: ({ status }) => `NeoDB 实例暂时不可用（HTTP ${status}），请稍后重试。`,
    loginStateMismatch: 'NeoDB 登录回调校验失败，请重新登录。',
    loginDenied: 'NeoDB 授权已被拒绝，请重新登录。',
    loginAuthorizeFailed: 'NeoDB 实例返回了授权错误，请重新登录。',
