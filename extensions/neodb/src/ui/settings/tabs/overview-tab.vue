...
import type { NeodbSettingsOverview } from '../../../shared/settings'
...
interface Props {
  overview: NeodbSettingsOverview
}
...
const accountLabel = computed(() => {
  if (!props.overview.account.configured) {
    return m.value.ui.overview.notSignedIn
  }
  return props.overview.account.instanceUrl ?? m.value.ui.overview.signedIn
})

const accountBadge = computed<{ label: string; variant: BadgeVariants['variant'] }>(() =>
  props.overview.account.configured
    ? { label: m.value.ui.overview.available, variant: 'success' }
    : { label: m.value.ui.overview.notSignedIn, variant: 'secondary' }
)
