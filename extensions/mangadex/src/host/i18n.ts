...
import { getMangadexMessages, type MangadexMessages } from '../shared/i18n'

/** Returns the message catalog for the current host UI locale. */
export function m(): MangadexMessages {
  return getMangadexMessages(kisaki.runtime.uiLocale)
}
 ...
 */
export function localizedMessage(select: (messages: MangadexMessages) => string): LocalizedText {
  const text: { en: string } & Partial<Record<UiLocale, string>> = {
    en: select(getMangadexMessages('en'))
  }
  for (const locale of UI_LOCALES) {
    if (locale === 'en') continue
    text[locale] = select(getMangadexMessages(locale))
  }
  return text
}
