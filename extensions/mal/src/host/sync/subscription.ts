...
/**
 * Debounced push of local media changes to the MAL lists.
 *
...
export class SyncSubscription implements Disposable {
  ...
  private async push(ref: LocalMediaRef, signal: AbortSignal): Promise<void> {
    ...
    if (result.status === 'synced') {
      this.deps.logger?.info('MAL automatic sync completed.', {
        kind: ref.kind,
        id: ref.id,
    ...
    this.deps.logger?.debug('MAL automatic sync skipped.', {
  ...
  private async handlePushError(
    ...
    this.deps.logger?.warn('MAL automatic sync failed.', {
      kind: ref.kind,
      id: ref.id,
    ...
    try {
      await kisaki.notify.error(m().sync.autoSyncFailedTitle, {
        ...
        id: `mal-auto-sync:${ref.kind}:${ref.id}`
      })
    } catch (notifyError) {
      this.deps.logger?.warn('MAL sync notification failed.', toSafeErrorLog(notifyError))
    }
  }
}
