 ...
 *
 * A person, company, or character is one entry read, and every slot is a
 * projection of it, so the read is memoized once per session and each slot
 * picks its field. The `images` slot name differs per media type (`photos` vs
 * `logos`), so the caller names it. A satellite whose slots need a second
 * source keeps its own session instead of bending this one.
 */

import type { ScraperSessionResult } from '@kisaki3/extension-sdk'
import type { BangumiSatelliteFacts } from './satellites'

export interface SatelliteSessionOptions<TSlot extends string, TInfo> {
  loadFacts(): Promise<BangumiSatelliteFacts<TInfo>>
...
export function createSatelliteSession<TSlot extends string, TInfo, TResultMap extends object>(
  ...
  let task: Promise<BangumiSatelliteFacts<TInfo>> | undefined
  const loadFacts = (): Promise<BangumiSatelliteFacts<TInfo>> => (task ??= options.loadFacts())
...
function readSlot<TSlot extends string, TInfo>(
  slot: TSlot,
  facts: BangumiSatelliteFacts<TInfo>,
  imageSlot: TSlot
): unknown {
  if (slot === imageSlot) {
    return facts.images
