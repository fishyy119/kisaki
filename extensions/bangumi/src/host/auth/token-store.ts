...
import { BANGUMI_SECRET_KEYS } from '../utils/ids'
...
export class TokenStore {
  ...
  async setPendingSession(session: Omit<BangumiPendingSessionSecretV1, 'version'>): Promise<void> {
    await this.secrets.set(
      BANGUMI_SECRET_KEYS.pendingSession,
      compactRecord({
        version: 1,
        ...session
      })
...
function normalizePendingSessionSecret(value: unknown): BangumiPendingSessionSecretV1 | undefined {
