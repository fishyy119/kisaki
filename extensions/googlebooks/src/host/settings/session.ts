...
import type { GbooksSettingsUiFunctions } from '../../shared/settings'
 ...
 * is attached.
 */
export class GbooksSettingsSession {
  private remote: WebviewRpcRemote<GbooksSettingsUiFunctions> | null = null

  constructor(private readonly logger: ExtensionLogger) {}

  attach(remote: WebviewRpcRemote<GbooksSettingsUiFunctions>): void {
    this.remote = remote
  }
  ...
  pushRefresh(reason: string): void {
    this.remote?.refreshRequested(reason).catch((error: unknown) => {
      this.logger.warn('Google Books settings refresh push failed.', {
        reason,
