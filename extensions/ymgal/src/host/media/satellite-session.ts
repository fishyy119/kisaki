 ...
 *
 * A person, company, or character is one archive read, and every slot is a
...
import type { YmgalSatelliteFacts } from './satellites'

export interface SatelliteSessionOptions<TSlot extends string, TInfo> {
  loadFacts(): Promise<YmgalSatelliteFacts<TInfo>>
...
export function createSatelliteSession<TSlot extends string, TInfo, TResultMap extends object>(
  ...
  let task: Promise<YmgalSatelliteFacts<TInfo>> | undefined
  const loadFacts = (): Promise<YmgalSatelliteFacts<TInfo>> => (task ??= options.loadFacts())
...
function readSlot<TSlot extends string, TInfo>(
  slot: TSlot,
  facts: YmgalSatelliteFacts<TInfo>,
  imageSlot: TSlot
): unknown {
  if (slot === imageSlot) {
    return facts.images
