...
const PUSH_OPERATION = 'anilist.list-push'
 ...
 */
export class AnilistTasks {
  constructor(private readonly deps: AnilistTasksDependencies) {}
  ...
  async startImport(options: ImportOptions): Promise<{ runId: string }> {
    ...
    void this.runGuarded(handle, 'AniList list import failed.', async () => {
    ...
    void this.runGuarded(handle, 'AniList full push failed.', async () => {
      ...
      for (const [index, ref] of refs.entries()) {
        ...
        try {
          ...
          failed += 1
          this.deps.logger?.warn('AniList push failed for one entry.', {
            kind: ref.kind,
            id: ref.id,
  ...
  async getTaskState(runId: string): Promise<AnilistTaskStateView | null> {
    ...
    const state: AnilistTaskStateView = { runId: snapshot.id, status: snapshot.status }
  ...
  private async requireIdle(): Promise<void> {
    ...
    if (active.length > 0) {
      throw new AnilistExtensionError('operation_running', m().errors.operationRunning)
    }
  }
