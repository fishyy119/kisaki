...
import type { SteamSettingsOverview } from '../../../shared/settings'
import { m } from '../i18n'
import { AUTOMATION_STATUS_VARIANTS, getAutomationLabel, getAutomationStatusLabel } from '../labels'

type SettingsTabId = 'account' | 'import' | 'automation' | 'maintenance'

interface Props {
  overview: SteamSettingsOverview
}
...
const accountLabel = computed(() =>
  props.overview.account.keyConfigured
    ? m.value.ui.overview.keyConfigured
    : m.value.ui.overview.noKey
)
...
const accountBadge = computed<{ label: string; variant: BadgeVariants['variant'] }>(() =>
  props.overview.account.keyConfigured
    ...
    : { label: m.value.ui.overview.noKey, variant: 'secondary' }
)
  ...
  <div class="space-y-4">
    <SettingsSection :title="m.ui.overview.statusTitle">
      <div class="overflow-hidden rounded-md border border-border">
        <div class="grid -m-px lg:grid-cols-2">
          <button
            type="button"
