/**
 * Path confinement for extension-supplied paths.
 *
 * Extension ids and relative paths arrive from manifests, RPC params, and the
 * extension host, so every resolved path is checked against its allowed root
 * before a caller touches the filesystem.
 */
