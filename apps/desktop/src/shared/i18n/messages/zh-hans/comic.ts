...
export const comic = {
  ...
  chapters: {
    title: '单元',
    emptyTitle: '还没有卷或话',
    emptyHint: '扫描漫画文件夹或抓取元数据以生成单元列表',
    unnamedVolume: ({ number }: { number: string }) => `第 ${number} 卷`,
    unnamedChapter: ({ number }: { number: string }) => `第 ${number} 话`,
    entityLabel: '单元',
    read: '已读',
    unread: '未读',
    resumeAt: ({ page }: { page: number }) => `从第 ${page} 页继续`,
    markRead: '标记为已读',
    markUnread: '标记为未读',
    readUpdated: '阅读状态已更新',
    ...
    readAt: '读完时间',
    releaseDate: '发行日期',
    pageCount: ({ count }: { count: number }) => `${count} 页`,

    catchUp: {
      title: '将剩余单元标记为已读？',
      pendingCount: ({ count }: { count: number }) => `${count} 个单元尚未标记为已读`,
      hint: '仅记录阅读状态，不记录阅读时间',
      markAll: '全部标记',
      skip: '跳过',
      marked: ({ count }: { count: number }) => `已将 ${count} 个单元标记为已读`
    },

    addChapter: '添加单元',
    editChapter: '编辑单元',
    deleteChapter: '删除单元',
    chapterDeleted: '单元已删除',
    volumeNumberLabel: '卷号',
    chapterNumberLabel: '话数',
    numberPlaceholder: '可选',
    numberInvalid: '编号必须为正数',
    numberRequired: '单元需要卷号、话数或名称',

    syncFiles: '同步文件',
    syncCompleted: ({ chapters, files }: { chapters: number; files: number }) =>
      `已同步 ${chapters} 个单元、${files} 个文件`,
    syncFailed: '文件同步失败',
    syncUnrecognized: ({ count }: { count: number }) => `${count} 个文件无法识别卷号或话数`
  },
