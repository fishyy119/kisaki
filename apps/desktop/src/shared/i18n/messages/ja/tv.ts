...
export const tv = {
  ...
  seasons: {
    ...
    entityLabel: 'シーズン',
    specials: 'スペシャル',
    unnamed: ({ number }: { number: number }) => `シーズン ${number}`,
    seasonCount: ({ count }: { count: number }) => `${count} シーズン`,
    posterEntityLabel: 'シーズンポスター',
    airDate: '放送開始日',
    addSeason: 'シーズンを追加',
    editSeason: 'シーズンを編集',
    deleteSeason: 'シーズンを削除',
    seasonDeleted: 'シーズンを削除しました。',
    numberLabel: 'シーズン番号',
    numberHint: 'スペシャルには 0 を使用します。',
    numberInvalid: 'シーズン番号は 0 以上で入力してください。',
    numberTaken: 'そのシーズン番号は既に使われています。',
    nameLabel: 'タイトル',
    namePlaceholder: '空欄可',
  ...
  episodes: {
    ...
    syncCompleted: ({
      ...
      `${seasons} シーズン、${episodes} エピソード、${files} ファイル、${extras} 特典を同期しました。`,
    syncFailed: 'ファイルの同期に失敗しました。',
    syncUnrecognized: ({ count }: { count: number }) =>
      `${count} 個のファイルはシーズンと話数を読み取れませんでした。`
  },

  extras: {
    title: '特典',
    emptyTitle: '特典がありません',
    emptyHint: 'フォルダー内の予告編やメイキング映像がここに表示されます。',
    entityLabel: '特典',
    addExtra: '特典を追加',
  ...
  detail: {
    openTvDir: 'シリーズフォルダーを開く',
    tvDirNotSet: 'シリーズフォルダーが設定されていません。',
    watchStatus: '視聴ステータス'
  },

  filesConfig: {
    title: 'ファイル設定',
    tvDirLabel: 'シリーズフォルダー',
    tvDirPlaceholder: '未設定',
    selectDir: 'フォルダーを選択',
    tvDirHint:
      '同期はこのフォルダーをスキャンし、SxxEyy の番号でエピソードファイルを照合します。空欄の場合は完全に手動で管理します。変更を保存すると再同期します。'
  },
