/**
 * File record actions for one anime episode or extra.
 *
...
export interface AnimeFileRecordsOptions {
  /** Owner row (episode or extra) with its current file records. */
  owner: MaybeRefOrGetter<{ id: string; files: AnimeFileRecord[] } | null>
  /** File table the mutations target. */
  table: AnimeFileTable
  /** FK column scoping the primary election to the owner row. */
  ownerColumn: typeof animeEpisodeFiles.episodeId | typeof animeExtraFiles.extraId
  /** Ingest call attaching a picked path as a user-owned record. */
  attach: (ownerId: string, path: string) => Promise<IpcVoidResult>
}

export interface AnimeFileRecords {
  isAttaching: Ref<boolean>
  attachFile: () => Promise<void>
  setPrimary: (file: Pick<AnimeFileRecord, 'id' | 'isPrimary'>) => Promise<void>
  removeFile: (fileId: string) => Promise<void>
  saveNote: (fileId: string, note: string | null) => Promise<void>
}

export function useAnimeFileRecords(options: AnimeFileRecordsOptions): AnimeFileRecords {
  const { m } = useI18n()
  ...
  async function attachFile(): Promise<void> {
    ...
    try {
      ...
      notify.error(m.value.anime.files.attachFailed)
    } finally {
      isAttaching.value = false
    }
  }

  async function setPrimary(file: Pick<AnimeFileRecord, 'id' | 'isPrimary'>): Promise<void> {
    const owner = toValue(options.owner)
...
export async function revealAnimeFile(path: string): Promise<void> {
  const { m } = useI18n()
