/**
 * File record actions for one series episode or extra.
 *
 * Owns manual attachment, primary election, record removal, and note saving
 * over the owner's file rows, so the episode and extra detail dialogs share
 * one mutation path. The target table and the ingest attach call are
...
import { tvEpisodeFiles, tvExtraFiles, type TvEpisodeFile, type TvExtraFile } from '@shared/db'
import type { IpcVoidResult } from '@shared/ipc'
import { useI18n } from './use-i18n'

const log = createLogger('Tv')

type TvFileRecord = TvEpisodeFile | TvExtraFile
type TvFileTable = typeof tvEpisodeFiles | typeof tvExtraFiles

export interface TvFileRecordsOptions {
  /** Owner row (episode or extra) with its current file records. */
  owner: MaybeRefOrGetter<{ id: string; files: TvFileRecord[] } | null>
  /** File table the mutations target. */
  table: TvFileTable
  /** FK column scoping the primary election to the owner row. */
  ownerColumn: typeof tvEpisodeFiles.episodeId | typeof tvExtraFiles.extraId
  /** Ingest call attaching a picked path as a user-owned record. */
  attach: (ownerId: string, path: string) => Promise<IpcVoidResult>
}

export interface TvFileRecords {
  isAttaching: Ref<boolean>
  attachFile: () => Promise<void>
  setPrimary: (file: Pick<TvFileRecord, 'id' | 'isPrimary'>) => Promise<void>
  removeFile: (fileId: string) => Promise<void>
  saveNote: (fileId: string, note: string | null) => Promise<void>
}

export function useTvFileRecords(options: TvFileRecordsOptions): TvFileRecords {
  const { m } = useI18n()
  ...
  async function attachFile(): Promise<void> {
    ...
    try {
      ...
      notify.success(m.value.tv.files.fileAttached)
    } catch (error) {
      log.error('File attach call threw:', error)
      notify.error(m.value.tv.files.attachFailed)
    } finally {
      isAttaching.value = false
    }
  }

  async function setPrimary(file: Pick<TvFileRecord, 'id' | 'isPrimary'>): Promise<void> {
    const owner = toValue(options.owner)
    ...
    try {
      ...
      notify.success(m.value.tv.files.primaryUpdated)
    } catch (error) {
  ...
  async function removeFile(fileId: string): Promise<void> {
    ...
    try {
      ...
      notify.success(m.value.tv.files.fileRemoved)
    } catch (error) {
  ...
  async function saveNote(fileId: string, note: string | null): Promise<void> {
    try {
      ...
      notify.success(m.value.tv.files.noteSaved)
    } catch (error) {
...
export async function revealTvFile(path: string): Promise<void> {
  const { m } = useI18n()
