/**
 * Reading state of one comic entry: the live reader window plus the start and
...
export function useComicReading(
  comicId: MaybeRefOrGetter<string>,
  chapterId?: MaybeRefOrGetter<string | undefined>
): ComicReading {
  const { m } = useI18n()
  const readingActivity = useReadingActivityStore()

  const isReading = computed(() => {
    const chapter = toValue(chapterId)
    return chapter
      ? readingActivity.getReadingChapterId(toValue(comicId)) === chapter
      : readingActivity.isComicReading(toValue(comicId))
  })
  ...
  async function stop(): Promise<void> {
    ...
    try {
      const result = await ipcManager.invoke('activity:stop-comic', toValue(comicId))
      if (!result.success) {
      ...
      log.error('comic stop call threw:', error)
