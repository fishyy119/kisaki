/**
 * Playback facade for one series extra.
 *
...
export interface TvExtraPlayback {
...
export function useTvExtraPlayback(extraId: MaybeRefOrGetter<string>): TvExtraPlayback {
  const { m } = useI18n()
  ...
  async function play(fileId?: string): Promise<void> {
    ...
    try {
      const result = await ipcManager.invoke('activity:play-tv-extra', toValue(extraId), fileId)
      if (!result.success) {
        notify.error(m.value.tv.extras.playFailed, result.error)
        return
      }
      if (result.data.status === 'failed') {
        notify.error(m.value.tv.extras.playFailed, m.value.activity.errors[result.data.reason])
      }
    } catch (error) {
      log.error('extra playback call threw:', error)
      notify.error(m.value.tv.extras.playFailed)
    } finally {
  ...
  async function stop(): Promise<void> {
    ...
    try {
      const result = await ipcManager.invoke('activity:stop-tv-extra', toValue(extraId))
      if (!result.success) {
        notify.error(m.value.tv.extras.stopFailed, result.error)
        return
      }
      if (result.data.status === 'failed') {
        notify.error(m.value.tv.extras.stopFailed, m.value.activity.errors[result.data.reason])
      }
    } catch (error) {
      log.error('extra stop call threw:', error)
      notify.error(m.value.tv.extras.stopFailed)
    } finally {
      pendingAction.value = null
    }
  }
