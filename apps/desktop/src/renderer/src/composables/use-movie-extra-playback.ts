/**
 * Playback facade for one movie extra.
 *
...
export interface MovieExtraPlayback {
...
export function useMovieExtraPlayback(extraId: MaybeRefOrGetter<string>): MovieExtraPlayback {
  const { m } = useI18n()
  ...
  async function play(fileId?: string): Promise<void> {
    ...
    try {
      const result = await ipcManager.invoke('activity:play-movie-extra', toValue(extraId), fileId)
      if (!result.success) {
        notify.error(m.value.movie.extras.playFailed, result.error)
        return
      }
      if (result.data.status === 'failed') {
        notify.error(m.value.movie.extras.playFailed, m.value.activity.errors[result.data.reason])
      }
    } catch (error) {
      log.error('extra playback call threw:', error)
      notify.error(m.value.movie.extras.playFailed)
    } finally {
  ...
  async function stop(): Promise<void> {
    ...
    try {
      const result = await ipcManager.invoke('activity:stop-movie-extra', toValue(extraId))
      if (!result.success) {
        notify.error(m.value.movie.extras.stopFailed, result.error)
        return
      }
      if (result.data.status === 'failed') {
        notify.error(m.value.movie.extras.stopFailed, m.value.activity.errors[result.data.reason])
      }
    } catch (error) {
      log.error('extra stop call threw:', error)
      notify.error(m.value.movie.extras.stopFailed)
    } finally {
      pendingAction.value = null
    }
  }
