<!--
Extension Webview Page renders one declared webview page full-page under its
stable `/extension-page/:extensionId/:pageId` route (the path and route name
are the webview navigation contract). The page is a pure container: the
document owns all chrome, and the app titlebar and sidebar stay as the
...
navigation binding.
Root `pages/` charter: this directory holds system-owned route surfaces only —
pages owned by core-level subsystems, importing no `features/*` code and
referenced by none. This page is owned by the `core/extensions` webview
runtime, not by the extension-manager feature.
-->
