<!--
  ...
  Dialog for editing person tags.
-->
...
import { tags, personTagLinks } from '@shared/db'
...
// Fetch person tags when dialog opens
const { data: results, isLoading } = useAsyncData(
  () =>
    db
      .select()
      .from(personTagLinks)
      .leftJoin(tags, eq(personTagLinks.tagId, tags.id))
      .where(eq(personTagLinks.personId, props.personId))
...
watch(results, (data) => {
  if (data) {
    items.value = data.map((row) => ({
      id: row.person_tag_links.id,
      tagId: row.person_tag_links.tagId,
      tagName: row.tags?.name || '',
      note: row.person_tag_links.note || '',
      isSpoiler: row.person_tag_links.isSpoiler,
      orderInPerson: row.person_tag_links.orderInPerson
    }))
  }
})
...
async function handleSave() {
  isSaving.value = true
  try {
    await db.delete(personTagLinks).where(eq(personTagLinks.personId, props.personId))
    for (let i = 0; i < items.value.length; i++) {
      const item = items.value[i]
      await db.insert(personTagLinks).values({
