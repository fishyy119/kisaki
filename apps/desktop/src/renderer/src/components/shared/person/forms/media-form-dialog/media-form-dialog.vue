<!--
  ...
  Dialog for managing person media (photo).
...
import PersonMediaSearchFormDialog from './media-search-form-dialog.vue'
import PersonMediaCropFormDialog from './media-crop-form-dialog.vue'
import PersonMediaUrlFormDialog from './media-url-form-dialog.vue'
...
const MEDIA_TYPES = computed<MediaTypeConfig[]>(() => [
  {
    ...
    description: m.value.library.forms.mediaDescriptions.personPhoto,
...
const selectedType = ref<PersonMediaType>('photo')
...
const {
  ...
  isLoading,
  refetch
} = useAsyncData(() => db.query.persons.findFirst({ where: eq(persons.id, props.personId) }), {
  watch: [() => props.personId],
  enabled: () => open.value
})

// Initialize person ref when data loads
watch(fetchedPerson, (data) => {
  person.value = data ?? null
})
...
const currentFile = computed(() => {
  ...
  return person.value[selectedConfig.value.field]
})
...
async function handleImportFile() {
  ...
  try {
    ...
    await attachment.setFile(persons, props.personId, selectedConfig.value.field, {
      kind: 'path',
...
async function handleDelete() {
  await attachment.clearFile(persons, props.personId, selectedConfig.value.field)
        ...
        <DialogBody class="flex gap-4">
          ...
          <div class="w-28 shrink-0 space-y-1">
            ...
            >
              ...
              >
                <img
                  v-if="person[media.field]"
                  :src="`attachment://persons/${person.id}/${person[media.field]}`"
                  :alt="media.label"
          ...
          <div class="flex-1 min-w-0 space-y-4">
            ...
            >
              <img
                ...
                :src="`attachment://persons/${person.id}/${currentFile}`"
