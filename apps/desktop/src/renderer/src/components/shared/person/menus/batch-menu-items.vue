<!--
  ...
  Menu items for batch operations on multiple persons.
-->
...
import { collections, collectionPersonLinks, persons } from '@shared/db'
...
async function fetchMenuData(): Promise<{ collections: CollectionRow[]; links: LinkRow[] }> {
  ...
  const links = await db
    ...
    .where(inArray(collectionPersonLinks.personId, ids))
...
const { data, refetch } = useAsyncData(fetchMenuData, {
  watch: [() => props.personIds, showNsfw],
...
const selectedEntityCount = computed(() => props.personIds.length)
    ...
    await db
      ...
      .where(
        and(
          ...
          inArray(collectionPersonLinks.personId, ids)
        )
      )
...
async function handleSetFavorite(isFavorite: boolean) {
  ...
  try {
    await db.update(persons).set({ isFavorite }).where(inArray(persons.id, ids))
