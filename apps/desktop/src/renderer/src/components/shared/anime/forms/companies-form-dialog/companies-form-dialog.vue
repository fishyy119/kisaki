<!--
  ...
  Dialog for editing anime companies grouped by role.
-->
...
import { animeCompanyLinks } from '@shared/db'
...
import AnimeCompaniesItemFormDialog from './company-item-form-dialog.vue'
...
// Fetch anime companies when dialog opens
const { data: results, isLoading } = useAsyncData(
  () =>
    db.query.animeCompanyLinks.findMany({
      where: eq(animeCompanyLinks.animeId, props.animeId),
      with: { company: true },
...
// Grouped companies by role
const groupedCompanies = computed(() => {
  const grouped = Object.fromEntries(
    COMPANY_ROLE_ORDER.map((role) => [role, [] as CompanyLinkItem[]])
  ) as Record<AnimeCompanyRole, CompanyLinkItem[]>
...
async function handleSave() {
  isSaving.value = true
  try {
    await db.delete(animeCompanyLinks).where(eq(animeCompanyLinks.animeId, props.animeId))
...
<template>
  <Dialog v-model:open="open">
    <DialogContent class="max-w-lg">
      ...
      <template v-else>
        <DialogHeader>
          <DialogTitle>{{ m.library.forms.editAnimeCompanies }}</DialogTitle>
        </DialogHeader>
