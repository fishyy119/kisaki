...
import { tags, animeTagLinks } from '@shared/db'
...
import AnimeTagsItemFormDialog from './tag-item-form-dialog.vue'
...
// Fetch anime tags when dialog opens
const { data: results, isLoading } = useAsyncData(
  () =>
    db
      .select()
      .from(animeTagLinks)
      .leftJoin(tags, eq(animeTagLinks.tagId, tags.id))
      .where(eq(animeTagLinks.animeId, props.animeId))
...
watch(results, (data) => {
  if (data) {
    items.value = data.map((row) => ({
      id: row.anime_tag_links.id,
      tagId: row.anime_tag_links.tagId,
      tagName: row.tags?.name || '',
      note: row.anime_tag_links.note || '',
      isSpoiler: row.anime_tag_links.isSpoiler,
      orderInAnime: row.anime_tag_links.orderInAnime
    }))
  }
})
...
async function handleSave() {
  isSaving.value = true
  try {
    await db.delete(animeTagLinks).where(eq(animeTagLinks.animeId, props.animeId))
    for (let i = 0; i < items.value.length; i++) {
      const item = items.value[i]
      await db.insert(animeTagLinks).values({
