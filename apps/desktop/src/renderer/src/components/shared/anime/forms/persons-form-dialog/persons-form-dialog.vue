<!--
  ...
  Dialog for editing anime staff credits grouped by role.
-->
...
import { animePersonLinks } from '@shared/db'
import type { AnimePersonRole } from '@shared/db'
...
import AnimePersonsItemFormDialog from './person-item-form-dialog.vue'
...
// Fetch anime persons when dialog opens
const { data: results, isLoading } = useAsyncData(
  () =>
    db.query.animePersonLinks.findMany({
      where: eq(animePersonLinks.animeId, props.animeId),
      with: { person: true },
...
// Grouped persons by role
const groupedPersons = computed(() => {
  const grouped = Object.fromEntries(
    PERSON_ROLE_ORDER.map((role) => [role, [] as PersonLinkItem[]])
  ) as Record<AnimePersonRole, PersonLinkItem[]>
...
async function handleSave() {
  isSaving.value = true
  try {
    await db.delete(animePersonLinks).where(eq(animePersonLinks.animeId, props.animeId))
...
<template>
  <Dialog v-model:open="open">
    <DialogContent class="max-w-lg">
      ...
      <template v-else>
        <DialogHeader>
          <DialogTitle>{{ m.library.forms.editAnimePersons }}</DialogTitle>
        </DialogHeader>
