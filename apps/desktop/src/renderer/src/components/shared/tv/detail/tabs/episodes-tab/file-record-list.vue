...
import type { TvEpisodeFile, TvExtraFile } from '@shared/db'

type FileRecord = TvEpisodeFile | TvExtraFile
...
function fileFacts(file: FileRecord): string {
  ...
  if (file.audioTracks.length > 0) {
    parts.push(m.value.tv.files.audioTrackCount({ count: file.audioTracks.length }))
  }
  if (file.subtitleTracks.length > 0) {
    parts.push(m.value.tv.files.subtitleTrackCount({ count: file.subtitleTracks.length }))
  }
  return parts.join(' · ')
}
</script>
  ...
  <div>
    ...
    >
      <DialogContent class="max-w-sm">
        <DialogHeader>
          <DialogTitle>{{ m.tv.files.editNote }}</DialogTitle>
        </DialogHeader>
        <Form @submit="handleNoteSubmit">
          <DialogBody>
            <Field>
              <FieldLabel>{{ m.tv.files.noteLabel }}</FieldLabel>
              <FieldContent>
    ...
    <DeleteConfirmDialog
      ...
      :entity-label="m.tv.files.recordEntityLabel"
      mode="remove"
