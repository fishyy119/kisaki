<!--
  ...
  Hero section for the series detail view: cover plus the personal engagement
  facts (last watched, status, duration, score), each editable on hover.
  Work metadata such as format and season counts lives in the overview tab.
-->
...
import { formatTvStatus, getEntityIcon } from '@renderer/utils/format'
import {
...
import type { TvStatus } from '@shared/db'
import TvWatchCatchUpDialog from '../tv-watch-catch-up-dialog.vue'
...
async function handleStatusSaved(status: string): Promise<void> {
  const entry = tv.value
  if (!entry) return

  try {
    catchUpOpen.value = await shouldOfferTvWatchCatchUp(entry.id, status as TvStatus)
  } catch (error) {
...
const coverUrl = computed(() =>
  tv.value?.coverFile
    ? getAttachmentUrl('tvs', tv.value.id, tv.value.coverFile, {
        width: 300,
        height: 400
      })
    : null
    ...
    <div class="flex-1 min-w-0 flex flex-col justify-between">
      <div>
        ...
        <div class="group/field relative flex items-center gap-3">
          <h2 class="text-xl font-bold truncate">{{ tv.name }}</h2>
          <Button
            variant="ghost"
            size="icon-xs"
      ...
      <div class="grid grid-cols-2 gap-x-8 gap-y-1.5">
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          ...
          <span class="font-medium truncate text-xs">
            {{ tv.lastActiveAt ? f.relativeTime(tv.lastActiveAt) : m.common.emptyValue }}
          </span>
        </div>
        ...
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          <span class="flex items-center gap-1.5 text-muted-foreground">
            ...
            <span class="text-xs">{{ m.tv.detail.watchStatus }}</span>
          </span>
          <span class="font-medium truncate text-xs">{{ formatTvStatus(tv.status) }}</span>
        </div>
        ...
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          ...
          <span class="font-medium truncate text-xs">
            {{ tv.totalDuration > 0 ? f.duration(tv.totalDuration) : m.common.emptyValue }}
          </span>
        </div>
        ...
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          ...
          <span class="font-medium truncate text-xs">
            {{ tv.score !== null ? (tv.score / 10).toFixed(1) : m.common.emptyValue }}
          </span>
        </div>
      </div>
    </div>
