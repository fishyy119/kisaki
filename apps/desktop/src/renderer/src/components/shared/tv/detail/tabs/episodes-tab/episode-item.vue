...
const title = computed(() => {
  ...
  const numbered =
    number === null ? null : m.value.tv.episodes.unnamed({ number: formatEpisodeNumber(number) })
    ...
    <div class="flex items-center gap-3 min-w-0">
      ...
      <div class="min-w-0">
        ...
        <div class="flex items-center gap-2 text-xs text-muted-foreground">
          <span v-if="!playableFile">{{ m.tv.files.missingFile }}</span>
          <template v-else>
            ...
            <template v-if="props.episode.files.length > 1">
              <span>·</span>
              <span>{{ m.tv.files.fileCount({ count: props.episode.files.length }) }}</span>
            </template>
          </template>
