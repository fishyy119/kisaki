<!--
  TvWatchButton
  Watch/stop button for a series entry or a specific episode. The watch action
  reads "start", "continue", or "next" depending on recorded watch progress, and
...
import { useTvWatch } from '@renderer/composables/use-tv-watch'
import { useI18n } from '@renderer/composables/use-i18n'
import { db } from '@renderer/core/db'
import { cn } from '@renderer/utils/cn'
import { tvEpisodes } from '@shared/db'

type WatchButtonState = 'idle' | 'starting' | 'playing' | 'stopping'

/** Which watch phase the entry or episode is in, before any transport starts. */
type WatchProgress = 'none' | 'resumable' | 'next'
...
/**
 * Which phase the watch action resumes: an episode button resumes from the
 * episode's own resume point, while an entry button resumes a partially watched
 * episode if one exists and otherwise moves on to the next unwatched episode.
 */
const progress = ref<WatchProgress>('none')

async function refreshProgress(): Promise<void> {
  const { tvId, episodeId } = props

  if (episodeId) {
    const [row] = await db
      .select({ resumePositionMs: tvEpisodes.resumePositionMs })
      .from(tvEpisodes)
      .where(eq(tvEpisodes.id, episodeId))
      .limit(1)
    progress.value = (row?.resumePositionMs ?? null) !== null ? 'resumable' : 'none'
    return
  }
  ...
  const [resumable] = await db
    ...
    .from(tvEpisodes)
    .where(and(eq(tvEpisodes.tvId, tvId), isNotNull(tvEpisodes.resumePositionMs)))
    .limit(1)
  if (resumable) {
    progress.value = 'resumable'
    return
  }
  ...
  const [watched] = await db
    .select({ id: tvEpisodes.id })
    .from(tvEpisodes)
    .where(and(eq(tvEpisodes.tvId, tvId), eq(tvEpisodes.watched, true)))
    .limit(1)
  progress.value = watched ? 'next' : 'none'
}

watch(
  () => [props.tvId, props.episodeId],
  () => void refreshProgress(),
  { immediate: true }
)

useDbChanges(({ table }) => {
  if (table === 'tv_episodes') void refreshProgress()
})

// Transitional phases keep the action label; the spinner alone signals progress.
const label = computed<string>(() => {
  if (isStopAction.value) return m.value.tv.stop
  if (progress.value === 'resumable') return m.value.tv.watchContinue
  return progress.value === 'next' ? m.value.tv.watchNext : m.value.tv.watchStart
})
...
async function handleClick(e: Event) {
  e.stopPropagation()
  e.preventDefault()
  await (isWatching.value ? stopTv() : watchTv())
}
</script>
