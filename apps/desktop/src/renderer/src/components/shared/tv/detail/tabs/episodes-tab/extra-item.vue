...
import type { TvExtraType } from '@shared/db'
...
const EXTRA_TYPE_ICONS: Record<TvExtraType, string> = {
...
const {
  ...
  play,
  stop
} = useTvExtraPlayback(() => props.extra.id)
</script>
      ...
      <div class="min-w-0">
        <div class="flex items-center gap-2">
          <span class="text-xs font-mono text-muted-foreground shrink-0">
            {{ m.library.tvExtraType[props.extra.type] }}
          </span>
          <p class="text-sm font-medium truncate">{{ props.extra.name }}</p>
        </div>

        <div class="flex items-center gap-2 text-xs text-muted-foreground">
          <span v-if="!primaryFile">{{ m.tv.files.missingFile }}</span>
          <template v-else>
            ...
            <template v-if="props.extra.files.length > 1">
              <span>·</span>
              <span>{{ m.tv.files.fileCount({ count: props.extra.files.length }) }}</span>
            </template>
          </template>
        </div>
