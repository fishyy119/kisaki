<!--
  NovelSelect
  Novel picker with built-in data fetching, in single or multiple mode.
-->
...
interface Props {
  ...
  disabled?: boolean
  /** Novel IDs to exclude from the list */
  excludeIds?: string[]
...
const placeholderText = computed(
  () =>
    props.placeholder ??
    m.value.library.select.searchPlaceholder({ label: m.value.library.entities.novel })
)
const emptyTextValue = computed(
  () =>
    props.emptyText ??
    m.value.library.select.selectPlaceholder({ label: m.value.library.entities.novel })
)
...
const novelEntities = useEntitySelectSource('novel', () => props.excludeIds)
