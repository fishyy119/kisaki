...
import { tvFilterQuerySpec } from '@shared/filter'
import type { FilterUiSpec } from './types'
import { getTvFormatOptions, getTvStatusOptions } from './shared-options'

export const tvFilterUiSpec: ComputedRef<FilterUiSpec<typeof tvFilterQuerySpec>> = computed(() => {
  ...
  return {
    ...
    fields: [
...

      {
        ...
        kind: 'enum',
        options: getTvStatusOptions()
      },
      {
        key: 'format',
        label: m.library.fields.format,
        kind: 'enum',
        options: getTvFormatOptions()
      },
      ...
      { key: 'totalSeasons', label: m.library.fields.totalSeasons, kind: 'number', min: 0 },
      ...
      { key: 'endDate', label: m.library.fields.endDate, kind: 'date' },
