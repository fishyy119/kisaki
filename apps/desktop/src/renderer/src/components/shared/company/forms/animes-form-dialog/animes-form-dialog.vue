<!--
  ...
  Dialog for editing company's related animes grouped by role.
-->
...
import { animeCompanyLinks, type AnimeCompanyRole } from '@shared/db'
...
import CompanyAnimesItemFormDialog from './anime-item-form-dialog.vue'
...
const COMPANY_ROLE_ORDER: AnimeCompanyRole[] = ['studio', 'producer', 'distributor', 'other']
...
// Fetch company animes when dialog opens
const { data: results, isLoading } = useAsyncData(
  () =>
    db.query.animeCompanyLinks.findMany({
      where: eq(animeCompanyLinks.companyId, props.companyId),
      with: { anime: true },
...
const groupedAnimes = computed(() => {
  const grouped = Object.fromEntries(
    COMPANY_ROLE_ORDER.map((role) => [role, [] as AnimeLinkItem[]])
  ) as Record<AnimeCompanyRole, AnimeLinkItem[]>
  items.value.forEach((item) => {
    grouped[item.role].push(item)
  })
  for (const role of COMPANY_ROLE_ORDER) {
    grouped[role].sort((a, b) => a.orderInCompany - b.orderInCompany)
  }
  return grouped
})
...
async function handleSave() {
  isSaving.value = true
  try {
    await db.delete(animeCompanyLinks).where(eq(animeCompanyLinks.companyId, props.companyId))
    ...
    if (items.value.length > 0) {
      ...
      if (linksToInsert.length > 0) {
        await db.insert(animeCompanyLinks).values(linksToInsert)
      }
    }
...
function handleItemFormSubmit(data: {
  ...
  const updatedItem: AnimeLinkItem = {
    ...
    orderInCompany: editingItem.value!.orderInCompany,
    isNew: editingItem.value!.isNew
  }

  if (isAddMode.value) {
    const roleLinks = groupedAnimes.value[updatedItem.role]
    updatedItem.orderInCompany = roleLinks.length
    ...
    if (index !== -1) {
      if (editingItem.value && editingItem.value.role !== updatedItem.role) {
        ...
        updatedItem.orderInCompany = newRoleLinks.length
      }
...
<template>
  <Dialog v-model:open="open">
    <DialogContent class="max-w-lg">
      ...
      <template v-else>
        <DialogHeader>
          <DialogTitle>{{ m.library.forms.editCompanyAnimes }}</DialogTitle>
        </DialogHeader>
