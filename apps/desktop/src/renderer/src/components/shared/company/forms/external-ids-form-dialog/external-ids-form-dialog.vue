<!--
  ...
  Dialog for editing company external IDs.
-->
...
import { companyExternalIds } from '@shared/db'
...
import CompanyExternalIdItem from './external-id-item.vue'
...
watch(results, (data) => {
  ...
  items.value = data.map((item) => ({
    ...
    orderInCompany: item.orderInCompany
  }))
})
...
async function handleSave() {
  ...
  try {
    ...
    await db.delete(companyExternalIds).where(eq(companyExternalIds.companyId, props.companyId))
...
<template>
  ...
  <CompanyExternalIdItemFormDialog
    v-if="itemFormOpen"
