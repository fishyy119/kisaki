<!--
  ...
  Dialog for editing character's related animes grouped by role type.
-->
...
import { animeCharacterLinks, type AnimeCharacterRole } from '@shared/db'
...
import CharacterAnimesItemFormDialog from './anime-item-form-dialog.vue'
...
// Fetch character animes when dialog opens
const { data: results, isLoading } = useAsyncData(
  () =>
    db.query.animeCharacterLinks.findMany({
      where: eq(animeCharacterLinks.characterId, props.characterId),
      with: { anime: true },
  ...
  if (data) {
    items.value = data
      ...
      .map((link) => ({
        ...
        animeCover: link.anime!.coverFile || null,
        role: link.role,
        note: link.note || '',
...
const groupedAnimes = computed(() => {
  const grouped = Object.fromEntries(
    CHARACTER_ROLE_ORDER.map((role) => [role, [] as AnimeLinkItem[]])
  ) as Record<AnimeCharacterRole, AnimeLinkItem[]>
...
<template>
  <Dialog v-model:open="open">
    <DialogContent class="max-w-lg">
      ...
      <template v-else>
        <DialogHeader>
          <DialogTitle>{{ m.library.forms.editCharacterAnimes }}</DialogTitle>
        </DialogHeader>
