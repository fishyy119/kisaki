...
import { movieExtras, MOVIE_EXTRA_TYPE_VALUES, type MovieExtraType } from '@shared/db'
...
type TypeChoice = MovieExtraType | 'auto'

const typeOptions = computed<{ value: TypeChoice; label: string }[]>(() => [
  ...(isEditing.value ? [] : [{ value: 'auto' as const, label: m.value.movie.extras.autoDetect }]),
  ...MOVIE_EXTRA_TYPE_VALUES.map((value) => ({
    value,
    label: m.value.library.movieExtraType[value]
  }))
])
...
async function saveCreate(): Promise<void> {
  const result = await ipcManager.invoke('ingest:attach-movie-extra-file', {
  ...
  notify.success(m.value.movie.extras.extraAttached)
  open.value = false
}
...
async function saveEdit(): Promise<void> {
  ...
  try {
    await db
      ...
      .where(eq(movieExtras.id, extra.id))
    notify.success(m.value.movie.extras.extraUpdated)
    open.value = false
  } catch (error) {
...
<template>
  <Dialog v-model:open="open">
    <DialogContent class="max-w-md">
      <DialogHeader>
        <DialogTitle>
          {{ isEditing ? m.movie.extras.editTitle : m.movie.extras.addExtra }}
        </DialogTitle>
      </DialogHeader>
      <Form @submit="handleSubmit">
        <DialogBody class="space-y-4">
          <Field v-if="!isEditing">
            <FieldLabel>{{ m.movie.files.title }}</FieldLabel>
            <FieldContent>
          ...
          <Field>
            <FieldLabel>{{ m.movie.extras.nameLabel }}</FieldLabel>
            <FieldContent>
              <Input
                v-model="name"
                :placeholder="isEditing ? undefined : m.movie.extras.autoDetect"
              />
            </FieldContent>
          </Field>

          <Field>
            <FieldLabel>{{ m.movie.extras.typeLabel }}</FieldLabel>
            <FieldContent>
              <Select v-model="type">
                <SelectTrigger>
                  <SelectValue />
