<!--
  MovieCard
  Pure display component for rendering a movie entry in lists/grids.
...
import { useMovieActivityStore } from '@renderer/stores'
...
import type { Movie } from '@shared/db'
...
const movieActivityStore = useMovieActivityStore()
const isWatching = computed(() => movieActivityStore.isMovieWatching(props.movie.id))

const imageUrl = computed(() =>
  props.movie.coverFile
    ? getAttachmentUrl('movies', props.movie.id, props.movie.coverFile, {
        width: 300,
        height: 400
      })
    : null
...
<template>
  ...
  >
    <MediaCard
      ...
      :image-url="imageUrl"
      :fallback-icon="getEntityIcon('movie')"
      :size="props.size"
      :align="props.align"
