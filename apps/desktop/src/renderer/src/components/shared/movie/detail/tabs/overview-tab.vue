...
import { MovieInfoFormDialog } from '../../forms'
import {
...
const { movie, tags, characters, persons, companies, relations } = useMovie()
const { m, f } = useI18n()
...
const PERSON_ROLE_LABELS = computed<Record<string, string>>(() => m.value.library.roles.moviePerson)
const COMPANY_ROLE_LABELS = computed<Record<string, string>>(
  () => m.value.library.roles.movieCompany
)
const CHARACTER_ROLE_LABELS = computed<Record<string, string>>(
  () => m.value.library.roles.movieCharacter
)
...
const hasExternalSites = computed(
  () => movie.value?.externalSites && movie.value.externalSites.length > 0
)

const sortedCharacters = computed(() =>
  [...characters.value]
    .sort((a, b) => {
      const roleIndexA = MOVIE_CHARACTER_ROLE_VALUES.indexOf(
        (a.role || 'other') as (typeof MOVIE_CHARACTER_ROLE_VALUES)[number]
      )
      const roleIndexB = MOVIE_CHARACTER_ROLE_VALUES.indexOf(
        (b.role || 'other') as (typeof MOVIE_CHARACTER_ROLE_VALUES)[number]
      )
      if (roleIndexA !== roleIndexB) return roleIndexA - roleIndexB
      return a.orderInMovie - b.orderInMovie
    })
    .map((link) => ({
      link,
  ...
  <template v-if="movie">
    <div class="grid md:grid-cols-[3fr_1fr] grid-cols-1 gap-8">
      <div class="space-y-6 min-w-0">
        ...
        >
          <MarkdownContent :content="movie.description!" />
        </Section>
      ...
      <div class="space-y-6 min-w-0">
        ...
        >
          <dl class="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-xs">
            ...
            <dd>{{ m.library.movieFormat[movie.format] }}</dd>
            <dt class="text-muted-foreground">{{ m.library.fields.runtime }}</dt>
            <dd>
              {{ movie.runtimeMs !== null ? f.duration(movie.runtimeMs) : m.common.emptyValue }}
            </dd>
            <dt class="text-muted-foreground">{{ m.library.fields.releaseDate }}</dt>
            <dd>{{ movie.releaseDate ? f.date(movie.releaseDate) : m.common.emptyValue }}</dd>
            <dt class="text-muted-foreground">{{ m.library.fields.addedDate }}</dt>
            <dd>{{ movie.createdAt ? f.date(movie.createdAt) : m.common.emptyValue }}</dd>
          </dl>
        </Section>
        ...
        >
          <div class="flex flex-col gap-1.5">
            <a
              v-for="(site, index) in movie.externalSites"
              :key="index"
              :href="site.url"
              target="_blank"
