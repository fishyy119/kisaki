<!--
  ...
  Hero section for the movie detail view: cover plus the personal engagement
  facts (last watched, status, duration, score), each editable on hover.
  Work metadata such as format and runtime lives in the overview tab.
-->
...
import { formatMovieStatus, getEntityIcon } from '@renderer/utils/format'
import {
    ...
    <div class="flex-1 min-w-0 flex flex-col justify-between">
      <div>
        ...
        <div class="group/field relative flex items-center gap-3">
          <h2 class="text-xl font-bold truncate">{{ movie.name }}</h2>
          <Button
            variant="ghost"
            size="icon-xs"
      ...
      <div class="grid grid-cols-2 gap-x-8 gap-y-1.5">
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          ...
          <span class="font-medium truncate text-xs">
            {{ movie.lastActiveAt ? f.relativeTime(movie.lastActiveAt) : m.common.emptyValue }}
          </span>
        </div>
        ...
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          <span class="flex items-center gap-1.5 text-muted-foreground">
            ...
            <span class="text-xs">{{ m.movie.detail.watchStatus }}</span>
          </span>
          <span class="font-medium truncate text-xs">{{ formatMovieStatus(movie.status) }}</span>
        </div>
        ...
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          ...
          <span class="font-medium truncate text-xs">
            {{ movie.totalDuration > 0 ? f.duration(movie.totalDuration) : m.common.emptyValue }}
          </span>
        </div>
        ...
        <div class="grid grid-cols-[auto_1fr] gap-3 items-center text-sm">
          ...
          <span class="font-medium truncate text-xs">
            {{ movie.score !== null ? (movie.score / 10).toFixed(1) : m.common.emptyValue }}
          </span>
        </div>
      </div>
    </div>
