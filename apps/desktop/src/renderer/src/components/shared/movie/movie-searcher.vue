<!--
  MovieSearcher
  Reusable movie search component.
...
import type { MovieScraperLookup, MovieSearchResult } from '@shared/scraper'
...
const emit = defineEmits<{
  selectionChange: [selection: EntitySearcherSelection<MovieScraperLookup>]
}>()

const selectedProfileId = ref('')
const searchProviderSource = useSearchProviderSource(selectedProfileId, 'movie')
 ...
 */
const searchResults = shallowRef<MovieSearchResult[]>([])
...
const movieId = ref('')
watch(
  () => props.defaultMovieId,
  (defaultId) => {
    if (defaultId !== undefined) {
      movieId.value = defaultId
    }
  },
  { immediate: true }
)
...
const canSubmit = computed(
  () => !!selectedProfileId.value && !!movieId.value.trim() && !props.isSubmitting
)

watch(
  [selectedProfileId, searchProviderSource, movieId, searchResults, selectedResultId],
  () => {
    const trimmedMovieId = movieId.value.trim()
    ...
    const fallbackKnownIds =
      searchProviderSource.value && trimmedMovieId
        ? [{ source: searchProviderSource.value, id: trimmedMovieId }]
        : []
    ...
    emit('selectionChange', {
      ...
      lookup: {
        ...
        // The picked row states which entry of the series this is, so providers
        // without a known id can tell it from sequels and remakes.
...
<template>
  <FieldGroup :class="props.class">
    ...
    <Field>
      <FieldLabel>
        {{ m.library.searcher.searchLabel({ label: m.library.entities.movie }) }}
      </FieldLabel>
      <FieldContent>
        <div class="flex gap-2">
          <Input
            v-model="searchQuery"
            :placeholder="m.library.searcher.namePlaceholder({ label: m.library.entities.movie })"
      ...
      >
        ...
        <template #state>
          <StateView
            ...
            size="sm"
            icon="icon-[mdi--magnify]"
            :description="m.library.searcher.startHint({ label: m.library.entities.movie })"
            class="h-full"
          />
    ...
    <Field>
      <FieldLabel>{{ m.library.searcher.idLabel({ label: m.library.entities.movie }) }}</FieldLabel>
      <FieldContent>
        <Input
          v-model="movieIdModel"
