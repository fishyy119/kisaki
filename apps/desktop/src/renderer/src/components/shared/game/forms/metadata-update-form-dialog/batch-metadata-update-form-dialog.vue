<!--
  ...
  Batch update game metadata from scraper results.
-->
...
const selectedFields = ref<GameMetadataUpdateField[]>([...GAME_METADATA_UPDATE_FIELDS])
const useCurrentExternalIdsAsKnownIds = ref(true)

const FIELD_LABELS: Record<GameMetadataUpdateField, string> = {
  name: '名称',
  originalName: '原名',
  releaseDate: '发售日期',
  description: '简介',
...
const { data, isLoading } = useAsyncData(
  async () => {
    ...
    const byGameId = new Map<string, ExternalId[]>()
    for (const ext of extRows) {
      const list = byGameId.get(ext.gameId) ?? []
      list.push({ source: ext.source, id: ext.id })
      byGameId.set(ext.gameId, list)
    }

    const out: BatchRow[] = rows.map((row) => ({
      id: row.id,
      name: row.name,
      originalName: row.originalName ?? null,
      externalIds: byGameId.get(row.id) ?? []
    }))
    return out
  },
  {
...
function toggleField(field: GameMetadataUpdateField, nextValue: boolean) {
...
function handleSelectAll() {
  selectedFields.value = [...GAME_METADATA_UPDATE_FIELDS]
}
...
async function handleSubmit() {
  ...
  const options = {
    fields: fieldsToOption(fields, GAME_METADATA_UPDATE_FIELDS),
  ...
  const toastId = notify.loading(toastTitle, `${entities.length} 个游戏`)
  ...
  if (failCount === 0) {
    ...
    const message = `成功 ${okCount}，失败 ${failCount}`
    notify.update(toastId, { title: '批量更新完成（部分失败）', message, type: 'warning' })
  }
          ...
          <DialogBody class="space-y-4 max-h-[70vh] overflow-y-auto scrollbar-thin">
            ...
            <FieldGroup>
              <Field>
                ...
                <FieldContent>
                  ...
                  <div class="grid grid-cols-2 gap-x-6 gap-y-2">
                    <div
                      v-for="field in GAME_METADATA_UPDATE_FIELDS"
                      :key="field"
              ...
              <Field orientation="horizontal">
                ...
                <FieldDescription>请勿在当前游戏为错误目标的情况下使用</FieldDescription>
              </Field>
            </FieldGroup>
          </DialogBody>
