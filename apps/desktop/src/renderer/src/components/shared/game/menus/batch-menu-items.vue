<!--
  GameBatchMenuItems
  Menu items for batch operations on multiple games.
-->
...
async function fetchMenuData(): Promise<{ collections: CollectionRow[]; links: LinkRow[] }> {
  ...
  const links = await db
    ...
    .where(inArray(collectionGameLinks.gameId, ids))
...
async function handleSetFavorite(isFavorite: boolean) {
  ...
  try {
    await db.update(games).set({ isFavorite }).where(inArray(games.id, ids))
