<!--
  GameCompaniesItem
  Single company item component showing icon, name, note with action buttons.
-->
...
const displayName = computed(() => (spoilerHidden.value ? '剧透内容' : props.companyName))
  ...
  <div class="flex items-center gap-3 p-3 rounded-lg border bg-card hover:bg-accent/30 group">
    <div class="shrink-0">
      <div class="size-10 rounded-md bg-muted flex items-center justify-center">
        <Icon
          :icon="spoilerHidden ? 'icon-[mdi--eye-off-outline]' : getEntityIcon('company')"
          class="size-5 text-muted-foreground"
        />
      </div>
    </div>
    <div class="flex-1 min-w-0">
      <div class="font-medium text-sm truncate flex items-center gap-2">{{ displayName }}</div>
      <div
        v-if="displayNote"
