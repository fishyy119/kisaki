<!--
  GameCharactersItem
  Single character item component showing avatar, name, note with action buttons.
-->
...
const photoUrl = computed(() =>
  props.characterPhoto
    ? getAttachmentUrl('characters', props.characterId, props.characterPhoto, {
        width: 100,
        height: 100
      })
    : null
...
const displayName = computed(() => (spoilerHidden.value ? '剧透内容' : props.characterName))
  ...
  <div class="flex items-center gap-3 p-3 rounded-lg border bg-card hover:bg-accent/30 group">
    ...
    >
      <div class="size-10 rounded-md bg-muted flex items-center justify-center">
        <Icon
          :icon="spoilerHidden ? 'icon-[mdi--eye-off-outline]' : getEntityIcon('character')"
