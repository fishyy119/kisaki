<!--
  GamePersonsItem
  Single person item component showing avatar, name, note with action buttons.
-->
...
const displayName = computed(() => (spoilerHidden.value ? '剧透内容' : props.personName))
  ...
  <div class="flex items-center gap-3 p-3 rounded-lg border bg-card hover:bg-accent/30 group">
    <div class="shrink-0">
      <div class="size-10 rounded-md bg-muted flex items-center justify-center">
        <Icon
          :icon="spoilerHidden ? 'icon-[mdi--eye-off-outline]' : getEntityIcon('person')"
