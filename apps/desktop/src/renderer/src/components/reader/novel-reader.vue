<!--
Novel reading shell: text volumes reflow through foliate, image-rendered PDF
volumes page through the same engine comics use, with the full image feature
set — layout, fit, zoom, display corrections, and the page grid.
Boundary: it reports positions; read state lives in the main process.
-->
<script setup lang="ts">
import { computed, onBeforeUnmount, onMounted, ref, shallowRef, watch } from 'vue'
import { storeToRefs } from 'pinia'
import type { NovelBookmark, NovelHighlight } from '@shared/db'
import type { ComicReadingDirection, HighlightColor } from '@shared/db/contracts/enums'
import { renderingOf } from '@shared/book'
import {
  formatPageLocator,
  parsePageLocator,
...
import { closeReaderWindow, reportProgress } from '@renderer/core/reader/bridge'
...
import { createNovelPdfPageSource, type PageSource } from '@renderer/core/reader/image/source'
...
import {
  ...
  type FoliateTocItem,
  type FoliateView
} from '@renderer/core/reader/text/foliate'
import { HIGHLIGHT_TINTS, resolveSearchTint } from '@renderer/core/reader/text/highlight'
import {
  applyNovelLayout,
...
import NovelMarksPanel from './novel-marks-panel.vue'
import FootnoteDialog from './text/footnote-dialog.vue'
import SearchPanel from './text/search-panel.vue'
import SelectionMenu from './text/selection-menu.vue'
import TypographyPopover from './text/typography-popover.vue'
...
const settingsStore = useReaderSettingsStore()
const { typography, pageDisplay, pageLayout, pageFit, autoCrop } = storeToRefs(settingsStore)
...
/** Session page flow of image-rendered volumes; novels store no override. */
...
/** Page progression of the open text volume, captured once its book exists. */
const bookRtl = ref(false)
...
const {
  ...
  open: async (target, isCurrent) => {
    if (renderingOf(target.container) === 'image') await openImageVolume(target, isCurrent)
    else await openTextVolume(target, isCurrent)
  }
})

const readable = computed(() => Boolean(unit.value?.fileId))
/** Which engine renders the open volume; PDF pages, everything else reflows. */
const rendering = computed(() => renderingOf(unit.value?.container ?? null))
const isImageVolume = computed(() => rendering.value === 'image')
const pageCount = computed(() => pageSource.value?.pageCount ?? null)

/** Volumes differ in what they can offer; the panel follows the open engine. */
const panelTabs = computed<ReaderPanelTab[]>(() =>
  isImageVolume.value ? ['outline', 'pages', 'marks'] : ['outline', 'search', 'marks']
)
...
const outline = computed<ReaderOutlineEntry[]>(() =>
  isImageVolume.value ? pdfOutline.value : flattenTocOutline(toc.value)
)

/**
 * Reading direction of the text surface: a forced vertical mode reads
 * right-to-left by construction, otherwise the book's own progression rules.
 */
const textRtl = computed(() => typography.value.writingMode === 'vertical' || bookRtl.value)
...
onMounted(() => {
  pageFlow.value = props.bootstrap.pageFlow
  void loadMarks()
...
async function openImageVolume(target: ReaderUnit, isCurrent: () => boolean): Promise<void> {
  const fileId = target.fileId
  if (!fileId) throw new Error('Novel unit has no readable file')
  ...
  if (isCurrent()) pdfOutline.value = entries
}

async function openTextVolume(target: ReaderUnit, isCurrent: () => boolean): Promise<void> {
  ...
  // leaves ordinary links to navigate the book. Links only exist in an opened
  // book, so a missing handle would be a foliate defect, not a state to serve.
  nextView.addEventListener('link', (event) => {
    if (!nextView.book) return
  ...
  bookRtl.value = nextView.book?.dir === 'rtl'

  const resumeLocator =
    target.resume?.kind === 'text' && !target.read ? target.resume.locator : null
  await nextView.init(resumeLocator ? { lastLocation: resumeLocator } : { showTextStart: true })
  if (!isCurrent()) return
...
/** Where an image-rendered volume resumes; text volumes carry a locator. */
...
function handleRelocate(volumeId: string, location: FoliateRelocation): void {
  ...
  if (fraction !== null && location.cfi) {
    reportProgress({
      unitId: volumeId,
      position: { kind: 'text', locator: location.cfi, fraction },
      extent: null
    })
  }
}
...
/** Text volumes navigate by TOC href, image-rendered ones by page index. */
...
function handleSeek(value: number): void {
  if (isImageVolume.value) engine.value?.setPage(value)
...
/** Left and right follow the effective writing mode, not the package alone. */
function textGoLeft(): void {
  if (typography.value.writingMode === 'vertical') void view.value?.renderer?.next()
  else void view.value?.goLeft()
}

function textGoRight(): void {
  if (typography.value.writingMode === 'vertical') void view.value?.renderer?.prev()
...
function handleKeydown(event: KeyboardEvent): void {
  ...
  switch (event.key) {
    case 'ArrowLeft':
      if (isImageVolume.value) engine.value?.turnLeft()
      else textGoLeft()
      break
    case 'ArrowRight':
      if (isImageVolume.value) engine.value?.turnRight()
      else textGoRight()
      break
    case ' ':
    case 'PageDown':
      if (isImageVolume.value) engine.value?.next()
      else void view.value?.renderer?.next()
      break
    case 'PageUp':
      if (isImageVolume.value) engine.value?.previous()
      else void view.value?.renderer?.prev()
      break
    case 'Home':
      if (isImageVolume.value) engine.value?.setPage(0)
      break
    case 'End':
      if (isImageVolume.value && pageCount.value !== null) {
        engine.value?.setPage(pageCount.value - 1)
      }
      break
    case '+':
    case '=':
      if (!isImageVolume.value) return
...
async function loadMarks(): Promise<void> {
  try {
    const [loadedBookmarks, loadedHighlights] = await Promise.all([
      fetchNovelBookmarks(props.bootstrap.entryId),
      fetchNovelHighlights(props.bootstrap.entryId)
    ])
...
function currentLocator(): string | null {
  if (isImageVolume.value) return formatPageLocator(pageIndex.value)
  return view.value?.lastLocation?.cfi ?? null
}

function currentProgressFraction(): number {
  if (!isImageVolume.value) return progressFraction.value
  const total = pageCount.value
  return total && total > 0 ? (pageIndex.value + 1) / total : 0
}
 ...
 * An image-rendered volume has no text layer and answers null.
 */
...
function applyTypography(): void {
  const renderer = view.value?.renderer
  if (!renderer) return

  renderer.setStyles?.(buildNovelContentStyles(typography.value))
  applyNovelLayout(renderer, typography.value)
}
</script>
...
<template>
  <div class="flex h-full flex-col overflow-hidden bg-background">
    <ReaderToolbar
      ...
      :paged="isImageVolume"
      :zoomable="isImageVolume"
      :searchable="!isImageVolume"
    ...
    >
      <template #controls>
        <template v-if="isImageVolume">
          <SettingsPopover v-model:page-flow="pageFlow" />
      ...
      <div class="relative min-w-0 flex-1 overflow-hidden">
        ...
        <template v-else>
          ...
          <div
            ...
            class="absolute inset-0 z-10 flex items-center justify-center bg-background"
          >
            <span class="text-sm text-muted-foreground">{{ m.reader.units.openFailed }}</span>
          </div>
