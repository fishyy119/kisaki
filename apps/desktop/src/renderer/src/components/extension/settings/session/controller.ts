...
export function useExtensionSettingsSession(
  contribution: Ref<ExtensionSettingsContributionInfo>,
  open: Ref<boolean>
): ExtensionSettingsSessionController {
  const session = ref<ExtensionSettingsSession | null>(null)
  const root = ref<SettingsSurfaceState<'root'> | null>(null)
  const activeDialog = ref<SettingsSurfaceState<'dialog'> | null>(null)
  const activeRootPopover = ref<SettingsSurfaceState<'popover'> | null>(null)
  const activeDialogPopover = ref<SettingsSurfaceState<'popover'> | null>(null)
  ...
  async function openRoot(): Promise<void> {
    ...
    try {
      const response = await invokeIpc<ExtensionSettingsOpenResponse>('extension:open-settings', {
        surface: 'root',
  ...
  async function closePopover(parent: ExtensionSettingsParentRef): Promise<void> {
  ...
  async function invokeNode(source: SettingsInvokeSource<ExtensionSettingsSurface>): Promise<void> {
    ...
    try {
      const response = await invokeIpc<ExtensionSettingsCallbackResponse>(
  ...
  async function submit(surface: SettingsSurfaceState<'root' | 'dialog'>): Promise<void> {
    if (!session.value) {
      return
    }
  ...
  async function applyCallbackResult(
    ...
    source: SettingsInvokeSource<ExtensionSettingsSurface>
  ): Promise<void> {
    if (!result.success) {
  ...
  async function openPopover(
    parentSurface: SettingsSurfaceState<ExtensionSettingsSurface>,
  ...
  async function refresh(
    target: string,
    reason?: ExtensionSettingsRefreshRequest['reason'],
    sourceSurface?: SettingsSurfaceState<ExtensionSettingsSurface>
  ): Promise<void> {
    if (!session.value || !root.value) {
      return
    }
  ...
  async function refreshRoot(reason?: ExtensionSettingsRefreshRequest['reason']): Promise<void> {
    if (!session.value || !root.value) {
      return
    }

    await closePopover({ surface: 'root' })
    const response = await invokeIpc<ExtensionSettingsRefreshResponse>(
  ...
  async function refreshDialog(reason?: ExtensionSettingsRefreshRequest['reason']): Promise<void> {
  ...
  async function refreshPopover(
    popover: SettingsSurfaceState<'popover'>,
    reason?: ExtensionSettingsRefreshRequest['reason']
  ): Promise<void> {
    if (!session.value || !root.value) {
      return
  ...
  async function refreshAll(reason?: ExtensionSettingsRefreshRequest['reason']): Promise<void> {
    if (!session.value || !root.value) {
      return
    }
  ...
  function toSubmitRequest(
    surface: SettingsSurfaceState<'root' | 'dialog'>
  ): ExtensionSettingsSubmitRequest {
    if (!session.value) {
      throw new Error('Settings session is not active.')
    }
  ...
  ): SettingsSurfaceState<'root' | 'dialog'> | null {
    if (parent.surface === 'root') {
      return root.value
    }
  ...
  function getCurrentSurface(
    surface: SettingsSurfaceState<ExtensionSettingsSurface>
  ): SettingsSurfaceState<ExtensionSettingsSurface> | null {
    if (surface.surface === 'root') {
      return root.value
    }
...
function getSettingsNodeCallbackId(node: ExtensionResolvedSettingsNode): string | undefined {
  return 'callbackId' in node ? node.callbackId : undefined
}

async function releaseSession(request: ExtensionSettingsReleaseRequest): Promise<void> {
  try {
    await invokeIpc<void>('extension:release-settings', request)
  } catch (e) {
    console.warn('[ExtensionSettingsSession] Failed to release settings session:', e)
  }
}

async function invokeIpc<T>(
  channel: 'extension:open-settings',
  request: ExtensionSettingsOpenRequest
): Promise<T>
async function invokeIpc<T>(
  channel: 'extension:refresh-settings',
  request: ExtensionSettingsRefreshRequest
): Promise<T>
async function invokeIpc<T>(
  channel: 'extension:submit-settings',
  request: ExtensionSettingsSubmitRequest
): Promise<T>
async function invokeIpc<T>(
  channel: 'extension:invoke-settings-node',
  request: ExtensionSettingsInvokeRequest
): Promise<T>
async function invokeIpc<T>(
  channel: 'extension:release-settings',
