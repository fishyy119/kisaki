<!--
  AnimeAdderDialog
  Dialog for adding anime to the library.
  Uses AnimeSearcher component for search and identification.
-->
...
import type { IngestAddAnimeFromScraperResult } from '@shared/ingest/add'

interface Props {
  /** Target collection ID to add the anime to */
...
const emit = defineEmits<{
  /** Called after the anime is successfully added */
  success: [animeId: string]
}>()

const { m } = useI18n()

const addFailedTitle = computed(() =>
  m.value.adder.addFailed({ label: m.value.library.entities.anime })
)
...
function handleSelectionChange(newSelection: AnimeSearcherSelection) {
...
async function handleSubmit() {
  ...
  const name = selection.value.originalName ?? selection.value.animeName
  ...
  try {
    ...
    if (run.status === 'cancelled') {
      notify.info(m.value.adder.addCancelled({ label: m.value.library.entities.anime }))
      return
    }
    ...
    const output = run.result?.output as IngestAddAnimeFromScraperResult | undefined
    if (!output?.animeId) {
      notify.error(
        addFailedTitle.value,
        m.value.adder.missingEntityId({ label: m.value.library.entities.anime })
      )
      return
    }

    emit('success', output.animeId)
  } catch (error) {
      ...
      <DialogHeader>
        <DialogTitle class="flex items-center gap-2">
          <Icon
            ...
            class="size-4"
          />
          {{ m.library.forms.addEntityTitle({ label: m.library.entities.anime }) }}
        </DialogTitle>
      </DialogHeader>
      <DialogBody>
        <AnimeSearcher
