...
const addFailedTitle = computed(() =>
  m.value.adder.addFailed({ label: m.value.library.entities.person })
)
...
async function handleSubmit() {
  ...
  try {
    ...
    if (run.status === 'cancelled') {
      notify.info(m.value.adder.addCancelled({ label: m.value.library.entities.person }))
      return
    }
    ...
    if (!output?.personId) {
      notify.error(
        ...
        m.value.adder.missingEntityId({ label: m.value.library.entities.person })
      )
      return
    }
      ...
      <DialogHeader>
        <DialogTitle class="flex items-center gap-2">
          <Icon
            ...
            class="size-4"
          />
          {{ m.library.forms.addEntityTitle({ label: m.library.entities.person }) }}
        </DialogTitle>
      </DialogHeader>
      <DialogBody>
        <PersonSearcher
