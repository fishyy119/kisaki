<!--
  ...
  Game ranking by play time or session count.
...
import { computeGameRanking, type RankingSort } from '@renderer/utils/statistics'
...
const rankingData = computed(() =>
  computeGameRanking(effectiveSessions.value, effectiveGames.value, sort.value).map((item) => ({
    id: item.id,
    name: item.name,
