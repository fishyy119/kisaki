...
import type { GameSaves } from '@main/services/attachment'
...
export class GameActivityCoordinator {
   ...
   * notifications and this coordinator logs the failure detail.
   */
  async launch(gameId: string): Promise<GameLaunchResult> {
    ...
    game.dirPath = launchConfig.dirPath
  ...
  private tapProcessHooks(): void {
    this.processService.hooks.processStarted.tap((payload) => {
      ...
      this.hooks.gamePlayStarted.dispatch({ gameId: tracked.gameId, pid: payload.pid })
    })
  ...
  private async savePlaySession(gameId: string, durationMs: number): Promise<void> {
    const record = await this.hooks.gamePlayEnding.transform({ gameId, durationMs })
    const now = Date.now()
  ...
  private async selectLauncherPath(game: Game): Promise<string | null> {
    ...
    const result = await this.native.dialogs.showOpenDialog({
      ...
      defaultPath: game.dirPath ?? undefined,
      properties: ['openFile'],
      filters
    })
    ...
    switch (game.launcherMode) {
      case 'file': {
        const result = await this.processService.launch.openFile(
          toAbsolutePath(launcherPath, game.dirPath)
        )
...
function resolveMonitorTarget(config: GameMonitorPathConfig): string | null {
  ...
  switch (config.monitorMode) {
    case 'folder':
      return config.dirPath ?? (config.launcherPath ? dirname(config.launcherPath) : null)
    case 'file':
