...
import type { ExtensionPackageRecoveryAction, ExtensionPackageRecoveryResult } from './types'
import {
  createRetainedArchiveSha256Set,
  inspectPackageDirectory,
  validatePackageForInstallation
} from './validation'
...
export async function recoverPackageTransactions(
  context: PackageTransactionContext
): Promise<ExtensionPackageRecoveryResult> {
  const { layout, db, archiveStore } = context
  await layout.ensureBaseDirectories()

  const actions: ExtensionPackageRecoveryAction[] = []
  const issues: string[] = []

  await pruneDirectoryChildren(layout.downloadsDir, (entryPath) => {
    actions.push({ type: 'pruned-download', path: entryPath })
  })
  await pruneDirectoryChildren(layout.stagingDir, (entryPath) => {
    actions.push({ type: 'pruned-staging', path: entryPath })
  })
  await pruneDirectoryChildren(layout.quarantineDir, (entryPath) => {
    actions.push({ type: 'pruned-quarantine', path: entryPath })
  })

  const installations = new ExtensionInstallationStore(db).list()
  const retainedArchiveSha256s = createRetainedArchiveSha256Set(installations)
  await archiveStore.pruneUnusedArchives(retainedArchiveSha256s, (entryPath) => {
  ...
  const activePackages = await scanPackageDirectories(layout.packagesDir)
  const backupPackages = await scanOperationPackages(layout.backupsDir)
  const trashPackages = await scanOperationPackages(layout.trashDir)
  ...
  for (const installation of installations) {
    const activeIssue = await validatePackageForInstallation(
      archiveStore,
      installation,
      layout.packageDir(installation.id)
    )
    if (!activeIssue) {
      ...
      for (const trash of trashPackages.filter((entry) => entry.extensionId === installation.id)) {
    ...
    if (recoverySource) {
      await fse.remove(layout.packageDir(installation.id)).catch(() => undefined)
      await fse.move(recoverySource.path, layout.packageDir(installation.id), {
        overwrite: false
      })
      actions.push({
...
async function findVerifiedRecoverySource(
  ...
  for (const candidate of candidates) {
    ...
    log.warn(
      `[ExtensionPackageTransactionCoordinator] Ignored invalid recovery package "${candidate.path}": ${issue}`
    )
  }
