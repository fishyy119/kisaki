 ...
 */
export function prepareRuntimeSlotConfigs<T extends ContentEntityType>(
  entityType: T,
  slotConfigs: ScraperProfile['slotConfigs'],
  providers: ReadonlyMap<string, RegisteredScraperProvider>
): SlotConfigsForEntityType<T> {
  const cleaned = normalizeSlotConfigs(entityType, slotConfigs)
  const slots = getScraperSlotsForEntityType(
    entityType
  ) as readonly (keyof SlotConfigsForEntityType<T> & ScraperSlot)[]
