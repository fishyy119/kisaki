...
import type { RegisteredScraperProvider } from './shared/registry'
...
export type ScrapeSlotOf<T extends ContentEntityType> = ScraperTypeMap[T]['slot'] & ScraperSlot
export type ScrapeImageSlotOf<T extends ContentEntityType> = ScraperTypeMap[T]['imageSlot'] &
  ScrapeSlotOf<T>
export type ScrapeLookupOf<T extends ContentEntityType> = ScraperTypeMap[T]['lookup'] &
  ScraperLookup
export type ScrapeSearchResultOf<T extends ContentEntityType> =
  ScraperTypeMap[T]['searchResult'] & { id: string }
export type ScrapeBundleOf<T extends ContentEntityType> = ScraperTypeMap[T]['bundle']
export type ScrapeResultMapOf<T extends ContentEntityType> = ScraperTypeMap[T]['resultMap'] &
  Partial<Record<ScrapeSlotOf<T>, unknown>>
export type ScrapeSessionOf<T extends ContentEntityType> = BaseScraperSession<
  ScrapeSlotOf<T>,
  ScrapeResultMapOf<T>
>
export type ScrapeProviderInfoOf<T extends ContentEntityType> = ScraperTypeMap[T]['providerInfo']
/** The info slot's payload; every entity's info names the entry (see conventions). */
export type ScrapeInfoOf<T extends ContentEntityType> = ScraperTypeMap[T]['resultMap']['info']

export type ScrapeProviderOf<T extends ContentEntityType> = ScraperTypeMap[T]['provider'] &
...
export type ScrapeResultOf<T extends ContentEntityType> = {
  [S in ScrapeSlotOf<T>]: SlotResult<S, ScrapeResultMapOf<T>[S]>
}[ScrapeSlotOf<T>]
export type ScrapeImageResultOf<T extends ContentEntityType> = {
  [S in ScrapeImageSlotOf<T>]: SlotResult<S, ScrapeResultMapOf<T>[S]>
}[ScrapeImageSlotOf<T>]

export interface ScraperEntitySpec<T extends ContentEntityType> {
...
} as const satisfies { [T in ContentEntityType]: ScraperEntitySpec<T> }
