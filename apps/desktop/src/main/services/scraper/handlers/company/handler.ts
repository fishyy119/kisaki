/**
 * Company Scraper Handler
 *
 * Profile-based company metadata scraping with multi-provider orchestration.
 */
...
export class CompanyScraperHandler {
  ...
  async ensureProfileValid(profileId: string): Promise<ProfileCleanupAction> {
    ...
    const cleanedConfigs = normalizeSlotConfigs('company', profile.slotConfigs)
    let hasChanges =
      Object.keys(profile.slotConfigs).some(
        (k) => !COMPANY_SCRAPER_SLOTS.includes(k as CompanyScraperSlot)
      ) || COMPANY_SCRAPER_SLOTS.some((slot) => !(slot in profile.slotConfigs))
    ...
    if (hasChanges) {
      this.db
        ...
        .run()
      log.info(`[Scraper] Updated company profile '${profile.name}' (cleaned invalid providers)`)
      return 'updated'
    }
  ...
  async scrape(
    ...
    const slotConfigs = normalizeSlotConfigs('company', profile.slotConfigs)
    ...
    if (searchResult) {
      ...
      if (searchResult.originalName) {
        ...
        log.info(`[Scraper] Using originalName '${searchName}' for cross-provider search (company)`)
      }
    }
  ...
  private async fetchAllSlots(
    ...
    const tasks: Promise<CompanyScraperResult | null>[] = []
    ...
    return results.filter((r): r is CompanyScraperResult => r !== null)
  }
  ...
  private buildCapabilities(provider: CompanyScraperProvider): ScraperCapability[] {
    ...
    if (provider.getLogos) caps.push('logos')
    return caps
  }
}
