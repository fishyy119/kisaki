/**
 * Character Scraper Handler
 *
 * Profile-based character metadata scraping with multi-provider orchestration.
 */
...
export class CharacterScraperHandler {
  ...
  async ensureProfileValid(profileId: string): Promise<ProfileCleanupAction> {
    ...
    const cleanedConfigs = normalizeSlotConfigs('character', profile.slotConfigs)
    let hasChanges =
      Object.keys(profile.slotConfigs).some(
        (k) => !CHARACTER_SCRAPER_SLOTS.includes(k as CharacterScraperSlot)
      ) || CHARACTER_SCRAPER_SLOTS.some((slot) => !(slot in profile.slotConfigs))
    ...
    if (hasChanges) {
      this.db
        ...
        .run()
      log.info(`[Scraper] Updated character profile '${profile.name}' (cleaned invalid providers)`)
      return 'updated'
    }
  ...
  async scrape(
    ...
    const slotConfigs = normalizeSlotConfigs('character', profile.slotConfigs)
    ...
    if (searchResult) {
      ...
      if (searchResult.originalName) {
        ...
        log.info(
          `[Scraper] Using originalName '${searchName}' for cross-provider search (character)`
        )
      }
    }
  ...
  private async fetchAllSlots(
    ...
    const tasks: Promise<CharacterScraperResult | null>[] = []
    ...
    return results.filter((r): r is CharacterScraperResult => r !== null)
  }
  ...
  private buildCapabilities(provider: CharacterScraperProvider): ScraperCapability[] {
    ...
    if (provider.getPersons) caps.push('persons')
