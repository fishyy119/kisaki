 ...
 * TV Scanner Handler
 *
 * Adds the tv-specific parts of a scan: resolving a directory to an existing
 * show, handing the match to ingest, and reconciling the season and episode
 * files inside the directory once the entry exists.
 */
...
export class TvScannerHandler extends MediaScannerHandler {
  constructor(
  ...
  protected findExistingByPath(path: string): { id: string; name: string } | undefined {
    return this.dbService.entityFinder.findExistingTv({ path })
  }
  ...
  protected async addDirect(
    ...
    const result = await this.ingestService.add.tv.addDirect(toSeed(match), toAddOptions(options))
    return { ...result, entityId: result.tvId }
  }
  ...
  protected async addFromScraper(
    ...
    const result = await this.ingestService.add.tv.addFromScraper(
      profileId,
      toSeed(match),
  ...
  protected async finalizeEntity(
    ...
    try {
      const sync = await this.ingestService.files.tv.sync({
        tvId: entityId,
        dirPath: entity.path,
        signal
      })
      ...
      log.warn('Failed to sync TV files after add.', error, { entityPath: entity.path })
