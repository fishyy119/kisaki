import type { ComicPlanContext, ComicUpdatePlan } from './types'
import { COMIC_LINK_TOPOLOGY, resolveLinkWrites } from '../link-topology'
import {
  areAliasesEqual,
  areExternalIdsEqual,
...
export function buildComicPlan(context: ComicPlanContext): ComicUpdatePlan {
  ...
  for (const surface of selection.coreSurfaces) {
    ...
    switch (surface) {
      ...
      case 'totalChapters': {
        ...
        const currentValue = current.comic[surface]
      ...
      case 'aliases': {
        const next = mergeAliases(
          current.comic.aliases,
          incoming.incoming.core.aliases ?? [],
          policy.collectionUpdate
        )
        if (!next) break
        if (areAliasesEqual(current.comic.aliases, next)) break
      ...
      case 'externalSites': {
        const next = mergeExternalSites(
          current.comic.externalSites ?? [],
          incoming.incoming.core.externalSites ?? [],
          policy.collectionUpdate
        )
        if (!next) break
        if (areExternalSitesEqual(current.comic.externalSites ?? [], next)) break
  ...
  if (selection.mediaSurfaces.includes('covers') && incoming.availability.surfaces.has('covers')) {
    ...
    if (shouldApplyMediaUpdate(current.comic.coverFile, coverUrl, policy.singularUpdate)) {
  ...
  if (
    ...
    if (shouldApplyMediaUpdate(current.comic.backdropFile, backdropUrl, policy.singularUpdate)) {
  ...
  if (selection.mediaSurfaces.includes('logos') && incoming.availability.surfaces.has('logos')) {
    ...
    if (shouldApplyMediaUpdate(current.comic.logoFile, logoUrl, policy.singularUpdate)) {
