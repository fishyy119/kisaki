...
import { applyComicPlan } from './apply'
import { loadComicCurrent } from './current'
import { buildComicIncoming } from './incoming'
import { buildComicPlan } from './plan'
import { COMIC_LINK_TOPOLOGY, createLinkDegradeWarnings } from './link-topology'
...
export class ComicUpdateHandler {
  constructor(
  ...
  startUpdateFromScraper(
    ...
    const run = createIngestRun(this.taskRunService, {
      operation: 'ingest.comic.update',
      title: this.i18nService.messages.ingest.update.title({ entity: 'comic' }),
      label: request.lookup.name,
      subject: { type: 'comic', id: request.rootId },
  ...
  async updateFromScraper(
    ...
    reportIngestProgress(options, {
      phase: 'preparing',
      label: this.i18nService.messages.ingest.update.preparing({ entity: 'comic' })
    })
    ...
    reportIngestProgress(options, {
      phase: 'scraping',
      label: this.i18nService.messages.ingest.update.scrapingMetadata({ entity: 'comic' })
    })
    const bundle = await this.scraperService.comic.scrape(request.profileId, lookup, {
      signal: options?.signal
    })
    throwIfIngestAborted(options?.signal)
    reportIngestProgress(options, {
      phase: 'planning',
      label: this.i18nService.messages.ingest.update.planning({ entity: 'comic' })
    })
    ...
    reportIngestProgress(options, {
      phase: 'writing',
      label: this.i18nService.messages.ingest.update.writing({ entity: 'comic' })
    })
    ...
    const { applyResult, degradedLinks } = this.dbService.client.transaction((tx) => {
      const current = loadComicCurrent(tx, request.rootId, selection)
      const plan = buildComicPlan({
        current,
        incoming,
        relationGraph,
        selection,
      ...
      return {
        applyResult: applyComicPlan(tx, request.rootId, plan, this.persistHandlers),
    ...
    if (warnings.length > 0) {
      log.warn('Comic update completed with warnings.', {
   ...
   */
  private resolveLookup(request: ComicUpdateRequest): ComicScraperLookup {
  ...
  private async handleUpdateFromScraperWithTaskRun(
    ...
    try {
      ...
      run.complete({
        title: this.i18nService.messages.ingest.update.completedTitle({ entity: 'comic' }),
        summary: this.i18nService.messages.ingest.update.completedSummary({ entity: 'comic' }),
        output: {
          comicId: request.rootId,
          ...result
        },
        counters: {
          updated: 1,
      ...
      if (isCancellation(error)) {
        run.cancel({
          summary: this.i18nService.messages.ingest.update.cancelledSummary({ entity: 'comic' })
        })
        return
      }
