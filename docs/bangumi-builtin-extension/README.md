...
不支持三次元，也不为未知媒体类型提前做抽象。现阶段 Kisaki 宿主只有游戏本地库写入、游戏 scraper provider、`game.*` host event 和 `kisaki.ingest.game.add.fromScraper`，因此 Bangumi 扩展采用以下结构：
...
- extension API 使用当前统一事件规范和目标 task run/automation 能力。Bangumi 扩展继续使用 `context.contributions.scraperProviders.game.register`、`kisaki.ingest.game.add.fromScraper`、`kisaki.library.games.*`、`kisaki.events.on('game.*')` 等现有入口。
...
- `kisaki.runtime.openExternal`、`kisaki.scrapers.profiles`、`kisaki.ingest.game.add.fromScraper`、`kisaki.commands`、`kisaki.taskRuns`、`kisaki.automations` 是目标宿主能力集合。
