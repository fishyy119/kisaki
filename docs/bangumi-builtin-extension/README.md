...
- extension API 使用当前统一事件规范。Bangumi 扩展继续使用 `context.contributions.scraperProviders.game.register`、`kisaki.ingest.games.addFromScraper`、`kisaki.library.games.*`、`kisaki.events.on('game.*')` 等现有入口。
