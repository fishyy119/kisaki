...
本文档集定义 `extensions/bangumi` 的目标形态和实施路线。Kisaki 是多媒体管理软件；Bangumi 扩展是 media scope 集成，支持 Bangumi 四类 subject，并通过目标公共 extension API 接入宿主。
...
不支持三次元，也不为未知媒体类型提前做抽象。现阶段 Kisaki 宿主只有游戏本地库写入、游戏 scraper provider、`game.*` host event 和 `kisaki.ingest.games.addFromScraper`，因此 Bangumi 扩展采用以下结构：
...
- extension API 使用 task-run-system 目标形态：`kisaki.taskRuns` 承载长时执行，`kisaki.automations` 承载持久自动化；不保留旧 alias 或兼容包装。
...
- [02-extension-system-integration.md](02-extension-system-integration.md): 接入目标 extension API、scoped TaskRun 和 Automation 能力。
...
- [06-settings-commands-and-tasks.md](06-settings-commands-and-tasks.md): settings panel、command job、TaskRun、Automation 与 UI 范围。
...
- extension API 使用当前统一事件规范和目标 task run/automation 能力。Bangumi 扩展继续使用 `context.contributions.scraperProviders.game.register`、`kisaki.ingest.games.addFromScraper`、`kisaki.library.games.*`、`kisaki.events.on('game.*')` 等现有入口。
...
- 长流程统一注册为 extension command；command handler 通过 `kisaki.taskRuns.create()` 创建 scoped TaskRun，settings panel 只启动 job，不直接执行长时间同步或导入。
...
- `kisaki.runtime.openExternal`、`kisaki.scrapers.profiles`、`kisaki.ingest.games.addFromScraper`、`kisaki.commands`、`kisaki.taskRuns`、`kisaki.automations` 是目标宿主能力集合。
- 命令注册是 contribution：`context.contributions.commands.register(...)`。命令执行是 capability：`kisaki.commands.start/execute/wait(...)`，长时执行状态由 command handler 创建的 TaskRun 承载。
- Deeplink contribution 返回 `urlPattern`，扩展局部 path 会被宿主归一化为 `kisaki://ext/<extensionId>/<path>`。
- DB event projector 已基于 SQLite trigger 的 OLD/NEW row snapshot 投影 `game.updated` 等 typed host event。
