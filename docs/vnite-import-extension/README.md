...
- **两段式导入**：先将 Vnite 数据规范化为 Kisaki 导入计划并直接导入，再按用户选择用 ingest 补全缺失元数据。
- **宿主事务写入**：新增通用 `kisaki.library.imports.applyGamePlan` capability，由宿主在主进程内完成事务、去重、关系写入、附件落盘和事件触发。
- **文件授权**：新增 `kisaki.files.openFile` capability。用户选中的 zip 会被复制到扩展临时目录，扩展只读取这个受控副本。
...
- [04 Import Model And Field Mapping](04-import-model-and-field-mapping.md): 导入 DTO、字段选择和 Vnite 到 Kisaki 的映射规则。
- [05 Import Flow And Metadata Completion](05-import-flow-and-metadata-completion.md): 端到端流程、导入算法、补全策略和冲突处理。
