...
- Kisaki targets eight root media types: game, anime, tv, movie, music, audio, comic, book. A new root type is justified only by a different consumption unit (session/episode/track/page), a different technical layer (process launch, video playback, audio playback, reading), or a different metadata graph; anything else is a `format` enum value on an existing type. Entry grain is a separate per-type decision: the entry sits where the domain puts strong identity, never inherited from the type that shipped first. `anime` entries are seasons and standalone works, with franchises as `media_relations` components; `tv` entries are shows, with seasons as weak child rows. See `references/conventions.md`.
