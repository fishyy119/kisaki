...
- Distinguish "unknown" from "known to be empty" across the scraper/ingest pipeline. A missing slot means the source could not answer; an empty collection means the source says there is none. Providers omit slots they cannot answer, merges preserve presence rather than collapsing empty arrays to `undefined`, and the `replace` collection policy may clear stored collections on an authoritative empty. Deleting needs stronger authority than writing: when several fact sources feed one link table, `replace` only clears once every source has answered, and degrades to `merge` with a warning otherwise. The link topology is declared once in `update/relation-links.ts`. See `references/architecture.md`.
- Kisaki targets eight root media types: game, anime, tv, movie, music, audio, comic, book. A new root type is justified only by a different consumption unit (session/episode/track/page), a different technical layer (process launch, video playback, audio playback, reading), or a different metadata graph; anything else is a `format` enum value on an existing type. See `references/conventions.md`.
...
- Logger prefixes are single-level stable domains such as `Extension`, `Db`, `Window`, `Updater`, `Scanner`, `Activity`, `Player`, `Process`, `Library`, `Theme`, `Hook`, `Ipc`, or `AsyncData`. Do not use dotted prefixes, `main`/`renderer`, file names, class names, function names, or dynamic ids as prefixes; pass dynamic values as log arguments.
