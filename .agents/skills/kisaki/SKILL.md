...
| Coding Conventions        | [conventions.md](references/conventions.md)           | Async patterns, error handling, naming claims, comments, i18n, git commit messages                     |
| Vocabulary                | [vocabulary.md](references/vocabulary.md)             | Canonical nouns, role nouns, sharing words, and the near-synonyms each excludes                        |
...
- Treat folders as either category organization or coupled module split. Category folders group peers and do not need a single entry; a per-entity folder family such as `activity/game/`, `activity/anime/` is a valid category, while a role-plural folder such as `handlers/` is not (see the folder predicate test in `references/architecture.md`). Coupled module folders split one module across files and should have a clear role entry such as `manager.ts`, `coordinator.ts`, `gateway.ts`, `provider.ts`, `point.ts`, `controller.ts`, `registry.ts`, `planner.ts`, or `view.ts`. Standalone cohesive single-file modules stay single-file instead of gaining a folder plus re-export `index.ts`; members of a templated collection (such as mirrored contribution points) keep the uniform folder shape even when currently single-file. See `references/architecture.md` for the full rule.
...
- Abstract mechanics, never domain meaning. Shared entity-generic code is parameterized only by schema facts (tables, columns) and injected functions, and helpers are opt-in for media types that fit. Flow ownership is earned: workflow ordering moves into a shared engine only after every shipped sample proved the flow invariant (the scan pipeline, ingest add/update engines, scraper handler, and unit-file reconcile are the precedents); unproved flow stays in explicit per-entity coordinators. Satellite entities (person/company/character) are cross-media shared and safe to dedupe; root media types grow one exemplar at a time (game, anime, comic, and novel shipped) — never extract generic root-media flows or a single all-consumer entity spec from one sample. Keep registries per consumer. See `references/conventions.md`.
...
- Logger prefixes are single-level stable domains such as `Extension`, `Db`, `Window`, `Updater`, `Scanner`, `Watch`, `Holdings`, `Activity`, `Video`, `Reader`, `Process`, `Library`, `Theme`, `Hook`, `Ipc`, or `AsyncData`. In main the domain is the service; in the renderer it is the feature area (`Library` for every entity's library UI, `Reader` for reader windows, `Extension`, `Scanner`, ...). Entity names (`Anime`, `Game`, `Tag`) are never prefixes: a log domain is a stable area of the application, not a row type. Do not use dotted prefixes, `main`/`renderer`, file names, class names, function names, or dynamic ids as prefixes; pass dynamic values as log arguments.
