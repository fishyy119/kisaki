...
- 调整生成的 GitHub provider 发布流程，改为基于 `<extension-id>-v<semver>` tag，不再依赖提交信息
...
- 调整生成的 GitHub provider 发布 workflow，不再识别 `publish(<extension-id>): v<semver>` 提交信息，发布入口改为 `<extension-id>-v<semver>` tag

## 迁移说明

- 要求在安装或发布扩展前，将本地和 CI 的 Node.js 运行时更新到 24 或更高版本
- 要求使用 GitHub provider 的生成仓库发布扩展时，先提交 manifest 版本更新并推送 `main`，再推送 `<extension-id>-v<semver>` tag；发布失败后可以将同名 tag 移到修复后的提交并重新推送
