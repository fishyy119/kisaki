...
export async function resolveProject(startDir = process.cwd()): Promise<ExtensionProject> {
  ...
  return {
    ...
    kisxConfigPath: path.join(rootDir, 'kisx.config.ts'),
    readmePath: path.join(rootDir, 'README.md'),
    srcDir: path.join(rootDir, 'src'),
    entrySourcePath: path.join(rootDir, 'src', 'host', 'index.ts'),
    uiSourceDir: path.join(rootDir, 'src', 'ui'),
